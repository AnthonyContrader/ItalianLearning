...
export class FindMistakeComponentsPage {
    createButton = element(by.id('jh-create-entity'));
    title = element.all(by.css('jhi-find-mistake div h2#page-heading span')).first();
...
export class FindMistakeUpdatePage {
    pageTitle = element(by.id('jhi-find-mistake-heading'));
    ...
    optionAInput = element(by.id('field_optionA'));
    optionBInput = element(by.id('field_optionB'));
    optionCInput = element(by.id('field_optionC'));
    ...
    async setOptionAInput(optionA) {
        await this.optionAInput.sendKeys(optionA);
    }

    async getOptionAInput() {
        return this.optionAInput.getAttribute('value');
    }

    async setOptionBInput(optionB) {
        await this.optionBInput.sendKeys(optionB);
    }

    async getOptionBInput() {
        return this.optionBInput.getAttribute('value');
    }

    async setOptionCInput(optionC) {
        await this.optionCInput.sendKeys(optionC);
    }

    async getOptionCInput() {
        return this.optionCInput.getAttribute('value');
    }
